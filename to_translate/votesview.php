...
if ($_GET[requestid])
{
	...
	$REL_TPL->stdhead("Votes");
	$res2 = sql_query("SELECT request FROM requests WHERE id=$requestid");
	$arr2 = mysql_fetch_assoc($res2);

	print("<h1>Voted for <a href=\"".$REL_SEO->make_link('requests','id',$requestid)."\"><b>$arr2[request]</b></a></h1>");
	print("<p>Vote <a href=\"".$REL_SEO->make_link('requests','action','vote','voteid',$requestid)."\"><b>request</b></a></p>");

	if (mysql_num_rows($res) == 0)
	print("<p align=center><b>Nothing there, man</b></p>\n");
	else
	{
		print("<table border=1 cellspacing=0 cellpadding=5>\n");
		print("<tr><td class=colhead>User</td><td class=colhead>Ratio</td></tr>\n");
